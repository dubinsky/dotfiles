...
Item {
  ...
  // Keep the Wayland idle-notify timeout in the range that actually fired
  // (150s). After shell.json raised screensaver/lock to 600/900, a 600s
  // compositor timeout produced zero idle events for >24h. QML timers cover
  // the rest of the configured delay.
  readonly property int compositorIdleSeconds: Math.min(60, firstIdleTimeoutSeconds)
  readonly property int screensaverDelaySeconds: Math.max(0, screensaverTimeoutSeconds - compositorIdleSeconds)
  readonly property int lockDelaySeconds: Math.max(0, lockTimeoutSeconds - compositorIdleSeconds)
  readonly property bool idleEnabled: stayAwakeStateLoaded && !stayAwake
  readonly property string screensaverClass: "org.omarchy.screensaver"
  readonly property string screensaverLauncher: home + "/.config/omarchy/plugins/dub.idle/launch-and-keep-focus"
  ...
  property bool idleMonitorArmed: true
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", root.screensaverLauncher)
  }

  function rearmIdleMonitor(reason) {
    logEvent("idle-monitor-rearm", reason || ("timeout=" + root.compositorIdleSeconds))
    root.idleMonitorArmed = false
    Qt.callLater(function() {
      root.idleMonitorArmed = true
  ...
  function startIdleCycle() {
    ...
    logEvent("idle-cycle-start", "screensaver=" + root.screensaverTimeoutSeconds + " lock=" + root.lockTimeoutSeconds + " compositor=" + root.compositorIdleSeconds)
  ...
  function statusJson() {
    return JSON.stringify({
      ...
      compositorIdle: root.compositorIdleSeconds,
  ...
  IdleMonitor {
    id: idleMonitor
    enabled: root.idleEnabled && root.idleMonitorArmed
    timeout: root.compositorIdleSeconds
    respectInhibitors: true
    onIsIdleChanged: root.handleIdleChanged()
  }

  onCompositorIdleSecondsChanged: {
    if (root.stayAwakeStateLoaded) root.rearmIdleMonitor("timeout-changed")
  }
